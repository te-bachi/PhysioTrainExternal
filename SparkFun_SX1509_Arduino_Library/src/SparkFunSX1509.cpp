...
byte SX1509::begin(TwoWire *wire, byte address, byte resetPin)
{
	// Store the received parameters into member variables
	deviceAddress =  address;
	pinReset = resetPin;
...
byte SX1509::readByte(byte registerAddress)
{
	...
	w->beginTransmission(deviceAddress);
	w->write(registerAddress);
	w->endTransmission();
	w->requestFrom(deviceAddress, (byte) 1);

	while ((w->available() < 1) && (timeout != 0))
		timeout--;
...
unsigned int SX1509::readWord(byte registerAddress)
{
	...
	w->write(registerAddress);
	w->endTransmission();
	w->requestFrom(deviceAddress, (byte) 2);

	while ((w->available() < 2) && (timeout != 0))
		timeout--;
...
void SX1509::readBytes(byte firstRegisterAddress, byte * destination, byte length)
{
	...
	w->beginTransmission(deviceAddress);
	w->write(firstRegisterAddress);
	w->endTransmission();
	w->requestFrom(deviceAddress, length);
	
	while (w->available() < length)
		;
	
	for (int i=0; i<length; i++)
	{
		destination[i] = w->read();
	}
}
