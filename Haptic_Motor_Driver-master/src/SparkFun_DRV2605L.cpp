...
extern TwoWire          muxWire;    //Da nicht standart Sercom

bool SFE_HMD_DRV2605L::begin() 
{
  //Wire.begin();                                                     //***************************************************************für Physiotrain auskommentieren
...
//    var = Wire.read();
    muxWire.beginTransmission(I2C_ADDR);
    muxWire.write((byte)reg);
    muxWire.endTransmission();
    muxWire.requestFrom((byte)I2C_ADDR, (byte)1);
    var = muxWire.read();
...
//    Wire.endTransmission();
    ...
    muxWire.beginTransmission(I2C_ADDR);
    muxWire.write((byte)where);
    muxWire.write((byte)what);
    muxWire.endTransmission();
}
