...
void SdFile::ls(uint8_t flags, uint8_t indent) {
  ...
  while ((p = readDirCache())) {
    ...
    for (int8_t i = 0; i < indent; i++) stream_->print(' ');
...
uint8_t SdFile::openRoot(SdVolume* vol, Stream* stream) {
    stream_ = stream;
    return openRoot(vol);
}
...
void SdFile::printFatDate(uint16_t fatDate) {
  stream_->print(FAT_YEAR(fatDate));
  stream_->print('-');
